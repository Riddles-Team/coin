...
define("DB_USERNAME", "tempcoin"); //Пользователь базы данных
define("DB_PASSWORD", "tempcointempcoin"); //Пароль от пользователя базы данных
define("DB_NAME", "tempcoin"); //Имя базы данных
define("APP_SECRET", "Lo0uWosvu3V6bHSC2DO2"); //Защищенный ключ приложения
define("SERVICE_KEY", "31c1df4a31c1df4a31c1df4aae31ac8f82331c131c1df4a6c5c7dcb05ebaa23b3aa0d26");
...
switch($method) {
    ...
    case 'user.buy.upgrade':
        ...
        if(isset($upgrade)) {
            ...
            if($user->clicks >= $price) {
               if($data['type'] === "0")  $user->speed = (float) $user->speed + (float) $upgrade->speed;
                ...
                $user = R::load('users', R::store($user));            
                $price_new = round((float) $price + ((float) $price / 0.25) * 5, 3);    
                ...
                $array = getUser($user, ['status' => 'error', 'msg' => "Недостаточно на балансе"]);
            }
        }
        $array = getUser($user);
        die(json_encode($array));
        ...
        break;
    case 'get.groups.top':
        $usersP = R::findAll('groups', "ORDER BY `clicks` DESC");
        $us = [];
        $i = 0;
    ...
    case 'get.upgrades.two':
        ...
        die(json_encode($usu));
        break;
    case 'get.upgrades.three':
        $user = R::findOne('users', 'user_vk_id = ?', [
            $data['user_vk']
        ]);
        if(!isset($user)) return;
        $upgrades = R::findAll('upgrades', "type = 2 ORDER BY `id` ASC");
        $usu = [];
        foreach ($upgrades as $key => $value) {
            $hist = R::findOne('history', "upgrade_id = ? AND user_id = ?", [$value->id, $user->id]);
    ...
    case 'user.auto.mine':
        ...
        if(isset($query_params['vk_group_id'])) {
            ...
            if($group==null) {
                ...
                $group->clicks = (float) $group->clicks + (float) $user->aspeed;
                R::load('groups', R::store($group));
            }
        }
